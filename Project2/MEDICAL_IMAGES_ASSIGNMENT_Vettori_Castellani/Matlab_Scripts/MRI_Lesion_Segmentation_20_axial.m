...
initial_slice=75;
axial_proj=get_slice(initial_slice,'axial',volume);
%% Cropping the slice

Crop = figure('Name','CropTheSlice');
Crop.WindowState = 'fullscreen';
[axial_crop, rectcrop]=imcrop(axial_proj);
close('CropTheSlice');

%% Treating the slice
[trtd_slice,area,mc,mr,sc,sr,br,bc]=treat_slice(axial_crop,-1,-1,nan);
...
show_scope(cor_proj,world_center_y,112-initial_slice,[1,1],[1,1]); %first is x second is y
[trtd_cor_slice,carea,cmc,cmr,csc,csr,cbr,cbc]=treat_slice(cor_proj,world_center_y,112-initial_slice,nan);
show_scope(trtd_cor_slice,cmc,cmr,cbr,cbc);
%% Replicate Area estimation process towards decreasing z
...
lower_boundary=112-cbr(2);
upper_boundary=112-cbr(1);

for i=initial_slice:-1:lower_boundary %initial_slice-6
   if(status==1)
   axial_i_projection=get_slice(i,'axial',volume);
   
   show_scope(axial_i_projection,new_center_x,new_center_y,[1,1],[1,1]);
   [elab_axial_slice,sarea,smc,smr,ssc,ssr,sbr,sbc,status]=treat_slice(axial_i_projection,new_center_x,new_center_y,radius);

   segmented_volume(:,:,i)=imrotate(double(axial_i_projection).*(elab_axial_slice+0.2),0);
...
%% Replicate Area estimation process towards increasing z
...
for i=initial_slice:+1:upper_boundary
   if(status==1)
   axial_i_projection=get_slice(i,'axial',volume);
...
[bin_slice]=get_circle_mask(bin_slice,c,r,radius);                         
end
%get the object
