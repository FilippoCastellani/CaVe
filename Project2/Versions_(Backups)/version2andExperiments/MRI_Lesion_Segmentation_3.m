...
subplot(121),imshow(sag_proj,'InitialMagnification', 'fit');
subplot(122),imhist(sag_proj); title('Histogram');
%close('SagittalView');                                                     %comment for visualization

%% Testing Edge() performance
figure
edges=edge(sag_crop,'canny');
%edges=imfill(edges);
imshow(edges);

figure
edges2=edge(sag_crop,'prewitt');
%edges2=imfill(edges2);
imshow(edges2);
...
%% Treating the slice
[trtd_slice,area,mc,mr,sc,sr,br,bc]=treat_slice(sag_crop,-1,-1);
...
show_scope(cor_proj,initial_slice,world_center_y,[1,1],[1,1]);
[trtd_cor_slice,carea,cmc,cmr,csc,csr,cbr,cbc]=treat_slice(cor_proj,initial_slice,world_center_y);
...
for i=initial_slice:-1:cbc(1) %initial_slice-6
   ...
   [elab_sag_slice,sarea,smc,smr,ssc,ssr,sbr,sbc,status]=treat_slice(sag_i_projection,new_center_x,new_center_y);
...
y(end-5:end)=0; %REMOVE THIS LINE
end
...
function [treated_slice,area,mu_col,mu_row,std_col,std_row,bou_row,bou_col,exit_status]=treat_slice(slice,r,c)
exit_status=1;
%enhancement
k=15;
g=0.2;
...
enh_slice2=imadjust(slice,[0 1],[0, 1],5);

figure
edges=edge(enh_slice,'canny');
edges2=edge(enh_slice2,'prewitt');
edges3=edges+edges2;
imshow(edges3);

%fill
fil_slice=imfill(edges3);
...
if (w==-1)&&(h==-1)
    ...
    imshow(bin_img,'InitialMagnification','fit');title('Binarized');
    [ms_x, ms_y]=getpts();
    bin_vec=bwselect(bin_img,ms_x,ms_y);
