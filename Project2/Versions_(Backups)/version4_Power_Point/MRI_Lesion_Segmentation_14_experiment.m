...
%% Compare manual segmentation vs automatic
load manual_segmented_sagittal_slice_135.mat
difference=double(sag_proj)-double(mask);
imshowpair(sag_proj,mask);

%% Create a manual segmentation
ManualSegmentation = figure('Name','Segment');
ManualSegmentation.WindowState = 'maximize';
imshow(sag_proj)
r=drawpolygon;
mask=createMask(r);
...
if (r==-1)&&(c==-1)
    ...
    imshow(imbinarize(slice),'InitialMagnification','fit');title('Click on the tumor');
    [c,r]=getpts();
end
