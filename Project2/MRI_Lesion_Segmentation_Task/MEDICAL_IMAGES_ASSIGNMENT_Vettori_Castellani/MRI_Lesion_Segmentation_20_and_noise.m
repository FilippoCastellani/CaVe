...
sag_proj=get_noised_slice(initial_slice,'sagittal',volume,ntype,mean,variance);
%% Cropping the slice manually
...
%% Cropping non manually
rectcrop=[1.285100000000000e+02,5.510000000000000,61.980000000000020,59.980000000000000]
[sag_crop]=imcrop(sag_proj,rectcrop);
...
cor_proj=get_noised_slice(world_center_x,'coronal',volume,ntype,mean,variance);
...
for i=initial_slice:-1:cbc(1) %initial_slice-6
   ...
   sag_i_projection=get_noised_slice(i,'sagittal',volume,ntype,mean,variance);
...
%% Get_Noised_Slice Function
function sl_img=get_noised_slice(sl_num,orientation,volume,ntype,mu,var)
switch orientation
    case 'sagittal'
        sag_proj=volume(sl_num,:,:);
        sag_proj_size=size(sag_proj);
...
if(ntype=='salt & pepper')
sl_img=imnoise(sl_img,ntype,mu);    
else
sl_img=imnoise(sl_img,ntype,mu,var);    
end
