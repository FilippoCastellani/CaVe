...
EEG_f=filtfilt(dn,double(EEG));
EEG_filt=filtfilt(pb_SOS,pb_G,EEGd);
...
pibe(1) = subplot(2,1,1); plot(t,EEG_filt,'b','linewidth',1); title('Filtered segment of EEG');
pibe(2) = subplot(2,1,2); 
%plot(t_k,D,'r','linewidth',0.2); title('PIB of EEG'); 
hold on
%plot(t_k,T,'b','linewidth',0.2);
%plot(t_k,A,'g','linewidth',0.2); 
%plot(t_k,B,'c','linewidth',0.2);
